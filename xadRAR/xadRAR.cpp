 ...
 , m_pArchive(nullptr)
{
	...
	m_XpkCaps.m_LibIdentifier = "RAR";
	m_XpkCaps.m_LibDescription = "RAR unpacking";
}
...
bool xadRAR::setArchive(QString &file)
{
	if (m_pArchive != nullptr)
	{
		delete m_pArchive;
	}
	m_pArchive = new CUnRAR(file.toStdString());
	return m_pArchive->List();
}

bool xadRAR::archiveInfo(QXpkLib::CArchiveInfo &info)
{
	if (m_pArchive == nullptr)
	{
		return false;
	...
	info.m_archiveInfo.m_packing = "RAR";
...
bool xadRAR::setExtractPath(QString &szPath)
{
	if (m_pArchive == nullptr)
	{
		return false;
...
// unpack/decompress
bool xadRAR::Decrunch(XpkProgress *pProgress)
{
	if (m_pArchive == nullptr)
	{
		return false;
