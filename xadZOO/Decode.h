...
protected:
	...
	CIOBuffer m_DictBuf; // dictionary/decrunch buffer
...
public:
    DecodeZoo()
     : m_crc()
     , m_BitIo()
     , m_DictBuf(16384*2) // guess something, grow if needed
    {}
    
	bool DecodeLzh(ZooEntry *pEntry);
	
	void setup(ZooEntry *pEntry, CIOBuffer *pIn, CIOBuffer *pOut)
	{
		...
		// whatever, ram is cheap..
		// just unpack and get it done
		//m_DictBuf.PrepareBuffer(pEntry->original_size, false);
	}
